			...
			<div class="flex items-center gap-3.5">
				...
				<div>
					...
					<p class="mt-0.5 text-xs text-slate-500">
						Your organization's active tier and allocated resource limits.
					</p>
				</div>
			</div>
				...
				<div class="rounded-xl border border-slate-100 bg-slate-50/50 p-4">
					<div class="flex items-center justify-between">
						...
						<span class="text-xs font-semibold text-slate-900">
							<span class="font-bold text-indigo-600">{formatQuotaVal(key, used)}</span> / {formatQuotaVal(
								key,
								limit
							)}
						</span>
			...
			<div class="flex items-center gap-2.5">
				...
				<div>
					...
					<p class="text-xs text-slate-500">
						Modules and platform capabilities enabled for this tier.
					</p>
				</div>
			</div>
		</div>
			...
			{:else}
				<div class="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
					{#each data.flags as flag (flag.key)}
						<div
							class="flex items-center justify-between rounded-lg border border-slate-100 p-3 {flag.enabled
						...
						>
							<div class="flex items-center gap-2">
								...
								/>
								<span class="font-mono text-xs font-semibold text-slate-800">{flag.key}</span>
							</div>
							<span
								class="text-[11px] font-bold {flag.enabled ? 'text-emerald-700' : 'text-slate-400'}"
							>
