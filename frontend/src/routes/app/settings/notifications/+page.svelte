...
<div class="space-y-6">
	...
	{#if prefsSaved}
		<div
			role="status"
			class="animate-fade-in flex items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-semibold text-emerald-800 shadow-2xs"
		>
	...
	<section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xs">
		<div
			class="flex flex-col gap-2 border-b border-slate-100 bg-slate-50/60 px-6 py-4 sm:flex-row sm:items-center sm:justify-between"
		>
			<div class="flex items-center gap-2.5">
				...
				<div>
					...
					<p class="text-xs text-slate-500">
						Real-time alerts shown in your top bell activity center.
					</p>
				</div>
			</div>
			<span
		...
		<div class="divide-y divide-slate-100">
			{#each inappPrefs as pref (pref.event_type)}
				{@const meta = EVENT_METADATA[pref.event_type] || {
		...
		>
			<div class="flex items-center gap-2.5">
				...
				<div>
					...
					<p class="text-xs text-slate-500">
						Direct transactional email summaries delivered to your inbox.
					</p>
				</div>
			</div>
			<span
