	...
	>
		...
		<section class="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs">
			...
			<div class="mt-5 space-y-4">
				<!-- Name -->
				<div>
					<label for="c-name" class="block text-xs font-semibold uppercase tracking-wider text-slate-700">
				...
				<!-- Client Type Cards -->
				<div>
					<span class="block text-xs font-semibold uppercase tracking-wider text-slate-700">Account Type</span>
					<div class="mt-1.5 grid grid-cols-2 gap-3">
						<label
							class="flex cursor-pointer items-center justify-between rounded-lg border p-3 transition-all {clientType === 'company' ? 'border-indigo-600 bg-indigo-50/40 ring-1 ring-indigo-600' : 'border-slate-200 hover:border-slate-300'}"
						>
						...
						<label
							class="flex cursor-pointer items-center justify-between rounded-lg border p-3 transition-all {clientType === 'individual' ? 'border-indigo-600 bg-indigo-50/40 ring-1 ring-indigo-600' : 'border-slate-200 hover:border-slate-300'}"
						>
							<div class="flex items-center gap-2.5">
								<input
									type="radio"
				...
				<div class="grid gap-4 sm:grid-cols-2">
					<div>
						<label for="c-email" class="block text-xs font-semibold uppercase tracking-wider text-slate-700">
							Contact Email
						</label>
						<input
							id="c-email"
					...
					<div>
						<label for="c-phone" class="block text-xs font-semibold uppercase tracking-wider text-slate-700">
							Phone Number
						</label>
						<input
							id="c-phone"
				...
				<!-- Tax ID -->
				<div>
					<label for="c-tax" class="block text-xs font-semibold uppercase tracking-wider text-slate-700">
		...
		<section class="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs">
			<div class="flex items-center gap-2.5 border-b border-slate-100 pb-4">
				...
				<div>
					...
					<p class="text-xs text-slate-500">Label and group this client (e.g. VIP, Retainer, Enterprise).</p>
				</div>
			</div>

			<div class="mt-5">
				<label for="c-tags" class="block text-xs font-semibold uppercase tracking-wider text-slate-700">Add Tags</label>
				<div class="mt-1.5">
					<input
						id="c-tags"
						type="text"
		...
		<section class="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs">
			<div class="flex items-center gap-2.5 border-b border-slate-100 pb-4">
				...
				<div>
					...
					<p class="text-xs text-slate-500">Provide direct login access for this client (optional).</p>
				</div>
			</div>
			...
			<div class="mt-5 space-y-4">
				<div class="grid gap-4 sm:grid-cols-2">
					<div>
						<label for="c-user-email" class="block text-xs font-semibold uppercase tracking-wider text-slate-700">
							Portal User Email
						</label>
						<input
							id="c-user-email"
					...
					<div>
						<div class="flex items-center justify-between">
							<label for="c-user-password" class="block text-xs font-semibold uppercase tracking-wider text-slate-700">
								Portal Password
							</label>
							<button
								type="button"
				...
				<p class="text-xs text-slate-500">Leave blank if this client does not require portal access yet.</p>
			</div>
		</section>
		...
		<div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
			<a
				...
				class="rounded-lg border border-slate-300 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-2xs hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
			>
				Cancel
			</a>
			<button
				...
				disabled={busy}
				aria-busy={busy}
				class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 transition-colors"
			>
