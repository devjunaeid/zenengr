...
<div class="space-y-6">
	...
	<div class="border-b border-slate-200">
		<nav class="-mb-px flex gap-6" aria-label="Role Scope Tabs">
			<button
				type="button"
				onclick={() => (activeRoleTab = 'user')}
				class="flex items-center gap-2 border-b-2 px-1 py-3 text-sm font-semibold transition-colors {activeRoleTab ===
				'user'
								...
								{:else}
									<div class="space-y-4">
										{#each filteredGroups as g (g.group)}
											{@const allGranted = g.items.every((c) =>
												draftGranted.has(`${c.action}.${c.resource}`)
											)}
											{@const someGranted = g.items.some((c) =>
												draftGranted.has(`${c.action}.${c.resource}`)
											)}
											...
											>
												...
												<div
													class="flex items-center justify-between border-b border-slate-100 bg-slate-50/70 px-4 py-2.5"
												>
													<div class="flex items-center gap-2">
...
<Dialog.Root open={newOpen} onOpenChange={(o) => (newOpen = o)}>
	<Dialog.Portal>
		...
		<Dialog.Content
			class="animate-in fixed top-1/2 left-1/2 z-50 flex max-h-[85vh] w-full max-w-xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-slate-100 bg-white shadow-2xl focus:outline-none"
		>
			<!-- Sticky Modal Header -->
			<div
