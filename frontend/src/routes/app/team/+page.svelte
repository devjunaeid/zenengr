<script>
	...
	let isFiltered = $derived(Boolean(searchQuery.trim() || filterRole !== 'all' || filterStatus !== 'all'));
...
<div class="flex items-center justify-between">
	...
	{#if canManageTeam}
		<button
			...
			class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 transition-colors"
		>
...
<div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-lg border border-slate-200 bg-white p-3.5 shadow-2xs">
	<!-- Search Box -->
	<div class="relative flex-1 min-w-[240px]">
		...
		<input
			...
			class="block w-full rounded-md border-slate-300 pl-9 pr-8 text-sm placeholder-slate-400 shadow-2xs focus:border-indigo-500 focus:ring-indigo-500"
		/>
		{#if searchQuery}
			<button
				type="button"
	...
	<div class="flex flex-wrap items-center gap-2.5">
		...
		{#if isFiltered}
			<button
				...
				class="inline-flex items-center gap-1 rounded-md px-2.5 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors"
			>
		...
		<table class="min-w-full divide-y divide-slate-200">
			<thead class="bg-slate-50">
				<tr>
					...
					>
					{#if canManageTeam}
						<th scope="col" class="px-4 py-3 text-right text-xs font-semibold tracking-wide text-slate-600 uppercase"
							>Actions</th
						>{/if}
				</tr>
			</thead>
					...
					<tr class="hover:bg-slate-50 transition-colors">
						<td class="px-4 py-3 text-sm font-medium text-slate-900">
							...
							{#if u.id === auth.user?.id}
								<span class="ml-1.5 rounded bg-indigo-50 px-1.5 py-0.5 text-xs font-semibold text-indigo-700">You</span>
							{/if}
						</td>
								...
								{#if u.id !== auth.user?.id}
									<div class="inline-flex items-center gap-1.5 justify-end">
										...
										<button
											...
											class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-2xs hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
										...
										{#if u.is_active}
											<button
												...
												class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-2xs hover:border-amber-200 hover:bg-amber-50 hover:text-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-500 disabled:cursor-not-allowed disabled:opacity-40 transition-all"
										...
										{:else}
											<button
												...
												class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 shadow-2xs hover:bg-emerald-100 hover:border-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
										...
										<button
											...
											class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-2xs hover:border-red-200 hover:bg-red-50 hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 disabled:cursor-not-allowed disabled:opacity-40 transition-all"
	...
	>
		<div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
			...
			>
				...
				<div>
					<label for="add-email" class="block text-sm font-medium text-slate-700">Email Address</label>
					<input
						id="add-email"
						type="email"
						bind:value={addEmail}
				...
				<div>
					<div class="flex items-center justify-between">
						<label for="add-password" class="block text-sm font-medium text-slate-700">Password</label>
						<button
							type="button"
	...
	>
		<div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
			...
			<p class="mt-2 text-sm text-slate-600">
				Setting new password for <span class="font-semibold text-slate-900">{passwordTarget.full_name}</span> ({passwordTarget.email}).
			</p>
			...
			>
				<div>
					<div class="flex items-center justify-between">
						<label for="new-pass" class="block text-sm font-medium text-slate-700">New Password</label>
						<button
							type="button"
