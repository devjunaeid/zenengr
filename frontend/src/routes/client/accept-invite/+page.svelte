<script>
	...
	async function submit() {
		...
		error = null;
		try {
			await portalAuth.register(fetch, {
	...
	<div class="w-full max-w-sm">
		<div class="rounded-lg border border-slate-200 bg-white p-8 shadow-sm">
			...
			{:else if data.state === 'expired'}
				<p class="mt-2 text-sm text-slate-500">
					This invite has expired. Ask your agency to resend the invite.
				</p>
			...
			{:else if data.state === 'ready' && data.invite}
				<p class="mt-2 text-sm text-slate-600">
					Join <span class="font-medium">{data.invite.client_name}</span> at
					<span class="font-medium">{data.invite.tenant_name}</span> as
