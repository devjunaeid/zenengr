...
def _resolve_logo_path(logo_url: Any) -> Path | None:
    """Resolve a /uploads/<filename> logo_url to a local file. None if missing."""
    if not isinstance(logo_url, str) or not logo_url:
        return None
    basename = Path(logo_url).name
    if not basename:
        return None
    candidate = Path(get_settings().uploads_dir) / basename
    return candidate if candidate.is_file() else None


def _draw_logo(canv: Any, doc: Any, logo_path: Path) -> None:
...
async def render_invoice_pdf(
    ...
    *,
    ...
    if logo_path is not None:
        doc.build(
            elements,
            onFirstPage=lambda canv, doc: _draw_logo(canv, doc, logo_path),
        )
    else:
        doc.build(elements)
    return buf.getvalue()
